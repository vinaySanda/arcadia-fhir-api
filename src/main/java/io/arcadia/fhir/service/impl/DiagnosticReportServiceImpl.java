...
public class DiagnosticReportServiceImpl implements DiagnosticReportService {
	private static final Logger logger = LoggerFactory.getLogger(DiagnosticReportServiceImpl.class);
	...
	@Override
	public DiagnosticReport getDiagnosticReportById(String id) {
	...
	public Bundle search(SearchParameterMap paramMap) {
		Map arcadiaResources = endPointResolver.getArcadiaResources(ResourceType.DiagnosticReport, paramMap);

		// filter logic

		return transformerUtils.transform(ResourceType.DiagnosticReport, arcadiaResources, paramMap);
	}
}
